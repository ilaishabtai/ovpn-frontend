...
export function ClientRow({ client, showServer, serverName }: Props) {
  return (
    <TableRow hover sx={{ "&:last-child td": { border: 0 }}}>
      ...
      <TableCell sx={{ py: 0.75 }}>
        <Typography variant="body2" fontFamily="monospace" color="primary.main" fontSize={14}>
          {client.virtual_ip ?? "—"}
        </Typography>
      </TableCell>

      <TableCell sx={{ py: 0.75 }}>
        <Typography variant="body2" fontFamily="monospace" color="text.secondary" fontSize={14}>
          {client.real_ip ?? "—"}
        </Typography>
      </TableCell>
