...
function ServerGroupRow({ serverName }: { serverName: string }) {
  return (
    <TableRow sx={{ backgroundColor: "rgba(56,189,248,0.04)" }}>
      <TableCell
        colSpan={9}
        sx={{ py: 0.75, borderBottom: "1px solid rgba(56,189,248,0.15)" }}
      >
    ...
    >
      <Table stickyHeader size="small">
        <TableHead>
          <TableRow>
            {columns.map((col) => (
              <TableCell key={col} sx={{ py: 1.25, fontWeight: 700 }}>
                {col}
              </TableCell>
            ))}
          </TableRow>
        ...
        <TableBody>
          {isGrouped &&
            servers!.map((server) => {
              const activeClients = Object.values(server.clients).filter(
                (c) => c.state === "active",
              );
          ...
          {isGrouped &&
            servers!.every((s) =>
              Object.values(s.clients).every((c) => c.state !== "active"),
            ) && (
              <TableRow>
                <TableCell
                  colSpan={7}
