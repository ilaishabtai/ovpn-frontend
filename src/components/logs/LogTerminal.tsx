    ...
    >
      ...
      <Box
        sx={{
          ...
          '&::-webkit-scrollbar-thumb': { background: 'rgba(255, 0, 0, 0.1)', borderRadius: 2 },
        }}
      >
        {isLoading && (
        ...
        {!isLoading && !isError && lines.map((line, idx) => (
          <Box
            key={idx}
            sx={{ display: 'flex', gap: 1.5, mb: 0.5, lineHeight: 1.7, fontSize: 12 }}
          >
            <Typography
              component="span"
              sx={{ color: 'rgba(255,255,255,0.2)', fontFamily: 'inherit', fontSize: 11, flexShrink: 0, whiteSpace: 'nowrap' }}
            >
              {line.timestamp}
            </Typography>
            <Typography
            ...
            <Typography
              component="span"
              sx={{ fontFamily: 'inherit', fontSize: 11, color: 'rgba(226,232,240,0.8)', wordBreak: 'break-word' }}
            >
              {line.message}
            </Typography>
          </Box>
        ...
        {!isLoading && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
            <Typography component="span" sx={{ color: '#cacaca', fontFamily: 'inherit', fontSize: 12 }}>user@ubuntu-badger:/ $</Typography>
            <Box
              sx={{
                width: 8,
                height: 14,
